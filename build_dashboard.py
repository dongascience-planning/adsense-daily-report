...
# 📅 8월 실행 계획 (WBS · 할 일 + 확인할 데이터). do=할 일, chk=확인할 데이터
# 방향: 효율은 7월에 잡았다 → 8월은 (질 좋은) 트래픽 + 충족률 방어
PLAN = [
    {"week": "🛡 안전장치 · 충족률·무효트래픽 감시 (7/29 교훈)", "days": [
        {"date": "상시", "label": "매일 충족률 감시 — 급락이 곧 경고",
         "do": ["매일 충족률 확인 — 70% 밑으로 떨어지면 ⚠️ (7/29~30엔 19%까지 폭락했었음)",
                "급락 시 GA4 유입 소스 점검 → 저질·봇 소스면 차단"],
         "chk": ["충족률이 80%대로 유지되나 (8월 현재 회복됨)"]},
    ]},
    {"week": "📈 핵심 레버 · 질 좋은 트래픽 확대", "days": [
        {"date": "8월", "label": "트래픽 = 남은 큰 레버 (효율은 7월에 천장 근처)",
         "do": ["터진 기사(예: 위고비) 패턴 분석 → 기획에 반영 (히트 재현)",
                "검색(SEO)·포털·SNS 유통 확대",
                "⚠️ 반드시 '질 좋은' 트래픽 — 저질 급증은 구글이 광고를 막아(7/29) 오히려 손해"],
         "chk": ["트래픽↑ 하면서 충족률·방문당 수익이 유지되나"]},
    ]},
    {"week": "⚙ 효율 잔여 최적화 (소소 · 큰 자리는 이미 잡힘)", "days": [
        {"date": "8월", "label": "아직 충족률 낮은 자리만 손질",
         "do": ["'닷컴-인기기사' 등 충족률 낮은 자리 반응형·형식 개선",
                "(여유 시) 자동광고 실험(Experiments) A/B로 미세 튜닝"],
         "chk": ["방문당 수익이 오르면서 이탈률은 유지되나"]},
    ]},
]
...
<body>
  ...
  <h2>📅 8월 실행 계획 <small>— 할 일 · 확인할 데이터 (체크하면 저장됨)</small></h2>
